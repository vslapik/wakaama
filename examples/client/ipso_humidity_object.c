/*
 * Implements an object for humidity sensor
 ...
 *  IPSO Humidity     | 3304 |    Yes    |    No     | Relative humidity sensor, example units = %|
 *
 *  Resources:
 *              Supported    Multiple
 ...
 * object - contain a chained list called instanceList with the object specific structure _ipso_humidity_instance_t:
 */
typedef struct
{
    /*
...
{
    _ipso_humidity_instance_t *targetP;
    double temp_humidity_value;

    targetP = (_ipso_humidity_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
    ...
    for (int i = 0 ; i < *numDataP ; i++)
    {
        ...
        case SENSOR_VALUE_ID:
            temp_humidity_value = (double) get_humidity();

            if (temp_humidity_value != DEFAULT_MEASURED_VALUE)
            {
                if ((targetP->min_measured_value == DEFAULT_MEASURED_VALUE) || (temp_humidity_value < targetP->min_measured_value)) 
                {
                    targetP->min_measured_value = temp_humidity_value;
                }

                if ((targetP->max_measured_value == DEFAULT_MEASURED_VALUE) || (temp_humidity_value > targetP->max_measured_value)) 
                {
                    targetP->max_measured_value = temp_humidity_value;
                }
            }

            targetP->sens_value = temp_humidity_value;
                        ...
                        lwm2m_object_t *objectP)
{
    ...
    case RESET_MIN_AND_MAX_MESURED_VALUES_ID:
        ...
        snprintf(uriBuffer, sizeof(uriBuffer), "/%d/%d/%d", IPSO_HUMIDITY_OBJECT_ID, instanceId, MIN_MEASURED_VALUE_ID);

        lwm2m_stringToUri(uriBuffer, sizeof(uriBuffer), &uri);
        lwm2m_resource_value_changed(*lwm2mContext, &uri);

        snprintf(uriBuffer, sizeof(uriBuffer), "/%d/%d/%d", IPSO_HUMIDITY_OBJECT_ID, instanceId, MAX_MEASURED_VALUE_ID);
...
void display_ipso_humidity_object(lwm2m_object_t *object)
{
#ifdef WITH_LOGS
    fprintf(stdout, "  /%u: Humidity object, instances:\r\n", object->objID);
    _ipso_humidity_instance_t *instance = (_ipso_humidity_instance_t *)object->instanceList;
    ...
    while (instance != NULL)
    {
        ...
        instance = (_ipso_humidity_instance_t *)instance->next;
    }
#endif
}
...
lwm2m_object_t *get_ipso_humidity_object(lwm2m_context_t **lwm2mH)
{
    ...
    if (NULL != tempObj)
    {
        ...
        tempObj->objID = IPSO_HUMIDITY_OBJECT_ID;

        targetP = lwm2m_malloc(sizeof(*targetP));
        if (NULL == targetP) return NULL;
        memset(targetP, 0, sizeof(*targetP));

        targetP->sens_value = get_humidity();
        strncpy(targetP->units, "\%", sizeof("\%"));
        targetP->max_measured_value = DEFAULT_MEASURED_VALUE;
        targetP->min_measured_value = DEFAULT_MEASURED_VALUE;

        //targetP->max_range_value = DHT11_max_hum_range_value;
        //targetP->min_range_value = DHT11_min_hum_range_value;
